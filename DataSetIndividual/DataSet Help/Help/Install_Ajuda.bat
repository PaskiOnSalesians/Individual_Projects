...
HelpLibraryManagerLauncher.exe /product "VS" /version "100" /locale es-es /uninstall /silent /vendor "Vendor Name" /productName "Ajuda per a crear el DataSet per a modificar uns registres" /mediaBookList "Ajuda per a crear el DataSet per a modificar uns registres" > NUL
...
REM build time.
IF EXIST "Ajuda.msha" COPY /Y "Ajuda.msha" HelpContentSetup.msha
...
REM Uninstall first in case it is already there.  If not, it won't install below.  We'll ignore any error output
REM by redirecting it to NUL.
HelpLibraryManagerLauncher.exe /viewerVersion %1  /locale es-es /wait 0 /operation uninstall /vendor "Vendor Name" /productName "Ajuda per a crear el DataSet per a modificar uns registres" /bookList "Ajuda per a crear el DataSet per a modificar uns registres" > NUL

REM Install the new content.
HelpLibraryManagerLauncher.exe /viewerVersion %1  /locale es-es /wait 0 /operation install /sourceUri "%CD%\Ajuda.msha"
